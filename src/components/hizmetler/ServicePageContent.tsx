import Link from "next/link";
import PageShell from "@/components/layout/PageShell";
import { getServicePage } from "@/lib/page-content";
import { notFound } from "next/navigation";

type ServicePageContentProps = {
  slug: string;
};

export default function ServicePageContent({ slug }: ServicePageContentProps) {
  const page = getServicePage(slug);

  if (!page) {
    notFound();
  }

  return (
    <PageShell narrow
      breadcrumbs={[
        { label: "Ana Sayfa", href: "/" },
        { label: "Hizmetler", href: "/hizmetler/" },
        { label: page.title },
      ]}
    >
      <article>
        <header className="mb-10">
          <h1 className="site-section__title text-left mb-4">{page.h1}</h1>
          <p className="text-muted text-lg leading-relaxed">{page.intro}</p>
        </header>

        <div className="space-y-10 text-muted leading-relaxed">
          {page.sections.map((section) => (
            <section key={section.heading}>
              <h2 className="text-xl sm:text-2xl font-semibold text-white mb-4">{section.heading}</h2>
              {section.paragraphs.map((paragraph) => (
                <p key={paragraph.slice(0, 40)} className="mb-4">
                  {paragraph}
                </p>
              ))}
              {section.list ? (
                <ul className="list-disc list-inside space-y-2 text-muted-secondary">
                  {section.list.map((item) => (
                    <li key={item}>{item}</li>
                  ))}
                </ul>
              ) : null}
            </section>
          ))}
        </div>

        <div className="mt-12 pt-8 border-t border-gray-800/50 flex flex-col sm:flex-row gap-4">
          <Link href="/iletisim/" className="site-btn-primary text-center">
            Ücretsiz Keşif Görüşmesi
          </Link>
          <Link href="/hizmetler/" className="site-btn-ghost text-center">
            Tüm Hizmetler
          </Link>
        </div>
      </article>
    </PageShell>
  );
}
